...
def draw_arch(ofa_net, resolution, out_name="viz/temp", info=None):
    ...
    with ddot.subgraph(name=model_name) as dot:
        ...
        dot.edge(
            ...
            "%s-%s" % (model_name, node_cnt),
            fontcolor="black",
            label=f'<<FONT POINT-SIZE="32">{base_stage_width[current_channel_size_idx]}x{curr_resolutions}x{curr_resolutions}</FONT>>',
        )
        # Residual connection
        dot.edge(
            "%s-%s" % (model_name, prev),
        ...
        for stage_id, block_idx in enumerate(ofa_net.block_group_info):
            ...
            for idx in active_idx:
                ...
                dot.node(
                    ...
                    width=str(w_lut[ofa_net.blocks[idx].conv.active_expand_ratio]),
                )
        ...
        dot.edge(
            ...
            "%s-%s" % (model_name, node_cnt),
            fontcolor="black",
            label=f'<<FONT POINT-SIZE="32">{base_stage_width[-2]}x{curr_resolutions}x{curr_resolutions}</FONT>>',
        )
        # global averaging
        # Final expand layer
        ...
        dot.edge(
            ...
            "%s-%s" % (model_name, node_cnt),
            fontcolor="black",
            label=f'<<FONT POINT-SIZE="32">{base_stage_width[-2]}x{1}x{1}</FONT>>',
        )
        # feature mix layer
        new_name = f"ConvLayer-{1}x{1}"
        dot.node(
        ...
        dot.edge(
            ...
            "%s-%s" % (model_name, node_cnt),
            fontcolor="black",
            label=f'<<FONT POINT-SIZE="32">{base_stage_width[-1]}x{1}x{1}</FONT>>',
        )
